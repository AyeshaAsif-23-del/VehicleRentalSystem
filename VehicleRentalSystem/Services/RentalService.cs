...
namespace VehicleRentalSystem.Services
{
    public class RentalService
    {
        ...
        public void Rent(int uid)
        {
            ...
            if (!int.TryParse(Console.ReadLine(), out int vid)) { UI.Error("Invalid ID."); return; }

            Console.Write(" Rental Days: ");
            if (!int.TryParse(Console.ReadLine(), out int days) || days <= 0) { UI.Error("Invalid days."); return; }
            ...
            if (!r.Read() || r.GetInt32(0) != 1) { UI.Error("Vehicle not available."); return; }
            ...
            cmd.CommandText = @"INSERT INTO Rentals(UserId,VehicleId,RentDate,DueDate,BaseRent,Status)
            ...
            UI.Success($"Booked! Total Rent: Rs {total}");
        }
        ...
        public void ProcessReturn(int uid, bool isAdmin = false)
        {
            ...
            {
                if (!reader.HasRows) { UI.Error("No active rentals found."); return; }
                Console.WriteLine("{0,-5} | {1,-15} | {2,-15}", "ID", "Vehicle", "Status");
                UI.DrawTableLine();
                while (reader.Read()) Console.WriteLine("{0,-5} | {1,-15} | {2,-15}", reader.GetInt32(0), reader.GetString(1), reader.GetString(2));
            }

            Console.Write("\n Enter Rental ID: ");
            if (!int.TryParse(Console.ReadLine(), out int rid)) return;

            // Settlement Logic (Late fine, damage etc)
            var cmd = conn.CreateCommand();
            cmd.CommandText = @"SELECT BaseRent, DueDate, VehicleId, (SELECT Value FROM SystemSettings WHERE Key='LateFeePerDay') FROM Rentals WHERE Id=@rid";
            cmd.Parameters.AddWithValue("@rid", rid);
            using var data = cmd.ExecuteReader();
            if (!data.Read()) { UI.Error("Rental not found."); return; }
            ...
            if (!isAdmin)
            {
                ...
                up.CommandText = "UPDATE Rentals SET Status='ReturnPending', LateFine=@lf, Total=@t WHERE Id=@rid";
            ...
            {
                Console.WriteLine($"\n[BILL] Base: {baseRent} | Late: {lateFine}");
                ...
                if (d == "1" || d == "2")
                {
                    ...
                    dCmd.CommandText = d == "1" ? "SELECT Value FROM SystemSettings WHERE Key='MinorDamage'" : "SELECT Value FROM SystemSettings WHERE Key='MajorDamage'";
                    damage = Convert.ToDouble(dCmd.ExecuteScalar());
                }

                double finalTotal = subTotal + damage;
                var up = conn.CreateCommand();
                up.CommandText = "UPDATE Rentals SET Status='Completed', DamageFine=@df, Total=@t, ReturnDate=@rd WHERE Id=@rid";
                ...
                UI.Success($"Settle! Total Paid: Rs {finalTotal}");
            }
        }
        ...
        public void ViewUserHistory(int uid)
        {
            ...
            conn.Open();
            var cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT v.Name, r.RentDate, r.Total, r.Status FROM Rentals r JOIN Vehicles v ON r.VehicleId=v.Id WHERE r.UserId=@u";
            cmd.Parameters.AddWithValue("@u", uid);
            ...
            while (r.Read())
            {
                ...
                Console.WriteLine(fmt, r.GetString(0), r.GetString(1), "Rs " + total, r.GetString(3));
            }
            Console.ResetColor();
            Console.ReadLine();
        }
